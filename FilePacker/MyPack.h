...
public:
	 ...
	 * \return 组指针
	 */
	MyGroup * getGrp(const char *grpName, ICheckSameGroup *check = NULL)
	{
...
class MyPack{
public:
	bool load(const char *packName);// 加载
	void unload(); // 卸载
	 ...
	 * \return 组指针
	 */
	MyGroup * getGrp(const char *grpName);
	/**
	 * 增加文件
	 * 传入文件的路径
	 * \return 成功 失败
	 ...
	 * 获取组下 文件的的内容
	 */
	MyFileBinary * getFileContent(const char *fileName);
