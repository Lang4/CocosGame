...
bool MyPack::load(const char *packName)// 加载
{
...
MyGroup * MyPack::getGrp(const char *grpName)
{
	for (GROUPS_ITER iter = groups.begin(); iter != groups.end();++iter)
	{
		if (iter->name == std::string(grpName))
		{
			return &(*iter);
		}
	}
 ...
 * 获取组下 文件的的内容
 */
MyFileBinary * MyPack::getFileContent(const char *fileName)
{
	...
	{
		MyGroup * group = getGrp(paths[0].c_str());
		for (unsigned int index = 1; index < paths.size();++index)
		{
			group = group->getGrp(paths[index].c_str());
		}
		if (group)
		{
			return group->getFileContent(winPath.c_str(),hFile);
		}
	}
	return NULL;
}